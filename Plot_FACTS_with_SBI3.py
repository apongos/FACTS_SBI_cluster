...
def FACTS(theta):
    ...
    # gFile='GesturalScores/KimetalOnlinepert3_DNN.G'
...
#             config['ArticStateEstimator']['estimated_somat_delay'] = str(theta[7].item())
    ...
    # formants_produced_record = np.full([last_frm+buffer_size,3], np.nan)
    ...
    # formants_produced_record_alltrials = np.full([ntrials,last_frm+buffer_size,3], np.nan)
    ...
    for trial in range(ntrials):
        ...
        if plot_trial:
            ...
            for i in range(len(Gest_states_labels)):
               # pdb.set_trace()
                plt.plot(all_X1[:, 7+i, 0], label=Gest_states_labels[i])
            plt.title('X1 velocity into TSE Aud Predict')
            plt.show()
            ...
            plt.figure()
            all_aps = np.array(model.task_state_estimator.all_eps)
            # pdb.set_trace()
            plt.plot(all_aps)
            plt.title(f'all_eps')
            plt.legend()
            ...
            plt.figure()
            all_Q = np.array(model.task_state_estimator.all_Q)
            # pdb.set_trace()
            for i in range(all_Q.shape[1]):
                plt.plot(all_Q[:,i,i])
            plt.title(f'all_Q[:,i,i]')
            plt.legend()
            #plt.show()

            plt.figure()
            all_R = np.array(model.task_state_estimator.all_R)
            # pdb.set_trace()
            for i in range(all_R.shape[1]):
                plt.plot(all_R[:,i,i])
            plt.title(f'all_R[:,i,i]')
            plt.legend()
            #plt.show()

            plt.figure()
            all_P = np.array(model.task_state_estimator.all_P)
            #pdb.set_trace()
            for i in range(all_P.shape[1]):
                plt.plot(all_P[1:,i,i])
            plt.title(f'all_P')
            plt.legend()
            #plt.show()
...
class Model():
    ...
    def tse_factory(self,tse_configs,R_Auditory,R_Somato):
        ...
        if model_type == 'lwpr':
            ...
            task_state_estimator = TSE_DNN(tse_configs)
...
class Hierarchical_Model(Model):
    ...
    def tse_factory(self,tse_configs,R_Auditory,R_Somato):
        ...
        if model_type == 'lwpr':
            ...
            task_state_estimator = TSE_DNN_Hier(tse_configs,R_Auditory,R_Somato)
        return task_state_estimator
...
class Hierarchical_xdotdot(Hierarchical_Model):
    def tse_factory(self,tse_configs,R_Auditory,R_Somato):
        ...
        if model_type == 'lwpr':
            ...
            task_state_estimator = TSE_DNN_Hier_xdotdot(tse_configs,R_Auditory,R_Somato)
        return task_state_estimator
...
class TSE_DNN(TaskStateEstimator):
    def __init__(self,tse_configs):
        self.Artic_to_Task_model = tf.keras.models.load_model(tse_configs['model_path'])
        self.a_mins = string2dtype_array(tse_configs['a_mins'],'float32')
        self.a_maxs = string2dtype_array(tse_configs['a_maxs'], 'float32')
        self.adot_mins = string2dtype_array(tse_configs['adot_mins'], 'float32')
        self.adot_maxs = string2dtype_array(tse_configs['adot_maxs'], 'float32')
        self.x_mins = string2dtype_array(tse_configs['x_mins'], 'float32')
        self.x_maxs = string2dtype_array(tse_configs['x_maxs'], 'float32')
        self.xdot_mins = string2dtype_array(tse_configs['xdot_mins'], 'float32')
        self.xdot_maxs = string2dtype_array(tse_configs['xdot_maxs'], 'float32')        
...
class TSE_DNN_Classic(TSE_DNN,TSEClassicInterface):
    def __init__(self,tse_configs):
        super().__init__(tse_configs)
        
    def run(self, a_tilde):
        # load tensorflow model
        a_position = a_tilde[0:gv.a_dim]
        a_velocity = a_tilde[gv.a_dim:2*gv.a_dim]
        # scale inputs
        input_i = np.zeros(2*gv.a_dim)
        input_i[0:gv.a_dim] = scale(a_position,self.a_mins,self.a_maxs)
        input_i[gv.a_dim:2*gv.a_dim] = scale(a_velocity,self.adot_mins,self.adot_maxs)
        input_i = input_i.reshape(1,len(input_i))
        # predict
        predict_task_state = np.squeeze(self.model.predict(input_i))
        scaled_x = predict_task_state[0:gv.x_dim]
        scaled_xdot = predict_task_state[gv.x_dim:2*gv.x_dim]
        #unscale
        x_position = unscale(scaled_x,self.x_mins,self.x_maxs)
        x_velocity = unscale(scaled_xdot,self.xdot_mins, self.xdot_maxs)
        x_tilde = np.append(x_position,x_velocity)
        return x_tilde

class TSE_DNN_Hier(TSE_DNN,TSEHierInterface):
    ...
    def run(self,a_tilde,formant_noise,i_frm,catch):
        ...
        # jac = self.Taskmodel.predict_J(a_tilde[0:gv.a_dim])
        # x_tilde = np.append(jac[0],np.matmul(jac[1],a_tilde[gv.a_dim:2*gv.a_dim]))
        a_tilde_shaped = np.reshape(a_tilde, (1, 12))
        pbd.set_trace()
        a_tilde_shaped = scale(a_tilde_shaped, )
        # x_tilde = self.Artic_to_Task_model(a_tilde_shaped)
        # x_tilde = x_tilde.numpy().flatten()
        # x_tilde = unscale(x_tilde, )
...
class TSE_DNN_Hier_xdotdot(TSE_DNN_Hier, TSEHierInterface):
    def __init__(self,tse_configs,R_Auditory,R_Somato):
...
#     return (point * (max-min) + min)
    ...
    def run(self,a_tilde_delaywindow,formant_noise,i_frm,catch,xdotdot):
        ...
        # jac = self.Taskmodel.predict_J(a_tilde[0:gv.a_dim])
        # x_tilde = np.append(jac[0],np.matmul(jac[1],a_tilde[gv.a_dim:2*gv.a_dim])) #KWANG's LWPR x_tilde  
        #pdb.set_trace()
        # Scale a_tilde before putting it in DNN
        a_tilde_pos = a_tilde[0:len(self.a_mins)]
        a_tilde_pos_scaled = scale(a_tilde_pos, self.a_mins, self.a_maxs)

        a_tilde_vel = a_tilde[len(self.adot_mins):]
        a_tilde_vel_scaled = scale(a_tilde_vel, self.adot_mins, self.adot_maxs)

        a_tilde_shaped = np.reshape(np.concatenate((a_tilde_pos_scaled, a_tilde_vel_scaled)), (1, 12))
       
       # Pass through artic-to-task and then unscale
        x_tilde_scaled = self.Artic_to_Task_model(a_tilde_shaped)
        x_minss = np.concatenate((self.x_mins, self.xdot_mins))
        x_maxss = np.concatenate((self.x_maxs, self.xdot_maxs))
        x_tilde_unscaled = unscale(x_tilde_scaled, x_minss, x_maxss)

        x_tilde = x_tilde_unscaled.numpy().flatten()
...
#             DeltaX, DeltaCov = seutil.StateCorrectionForDelay(X2, self.Wc, Y1, self.P, z, delay_y, self.cc_discount_from_delay)
            DeltaX, DeltaCov, self.K = seutil.StateCorrection(X2, self.Wc, Y1, self.P, z, delay_y)
            ...
            if len(self.all_eps) > accumulated_frames_threshold:
                if np.sum(np.array(self.all_eps)[-accumulated_frames_threshold:] > sensory_error_threshold) >= 1:# Look at window of eps how consistent
                    ...
                    if self.compensation_adaptive_frame_on == False:
                        ...
                        print(f'compensation_adaptive_frame_on i_frm:{i_frm} all_eps_i:{len(self.all_eps)}')
...
#                     DeltaX, DeltaCov = seutil.StateCorrection(X2*self.AUKF_Pmult_for_pert,self.Wc,Y1,obscov,z,delay_y)
...
#             self.P = P1 - DeltaCov#covariance updat
    ...
    def sigmoid_multiply(self, param_to_mult, upper_limit, ramp_rate, accumulated_frames_threshold):
        ...
        all_eps = np.array(self.all_eps)
        accumulated_frames_in_error = np.sum(all_eps[self.compensation_adaptive_frame_i_frm-accumulated_frames_threshold:]>self.sensory_error_threshold)
        new_param = param_to_mult * (upper_limit / (1 + ramp_rate ** - (accumulated_frames_in_error) ) - (upper_limit/2) ) #self.AUKFmultFactor[1] #commented 052522
        return new_param
...
#                 print('sensory_error')
            ...
            # Now do the AUKF for perturbation Alvince 2024
            sensory_error = z-delay_y
            # print(f'z {z} {len(self.all_eps)}')
            # print(f'delay_y {delay_y}')
            # pdb.set_trace()
            eps = np.matmul(np.matmul(np.transpose(sensory_error),np.linalg.inv(obscov)),sensory_error)
            #pdb.set_trace()
            sensory_error_threshold = self.sensory_error_threshold
...
#             print(f'self.Q {self.Q}')
            ...
            if len(self.all_eps) > accumulated_frames_threshold:
                if np.sum(np.array(self.all_eps)[-accumulated_frames_threshold:] > sensory_error_threshold) >= 1:# Look at window of eps how consistent
                    ...
                    #Q_amp_test = 1e-15
                    #P_amp_test = 3.5
                    # self.Q = self.defQ*1e-15
                    #self.Q = (self.defQ*0) + (self.defQ) * ( (Q_amp_test) / (1 + (1.01) **(-np.sum(np.array(self.all_eps)[self.compensation_adaptive_frame_i_frm-accumulated_frames_threshold:]>sensory_error_threshold) ) ) - (Q_amp_test/2) ) #self.AUKFmultFactor[1] #commented 052522
                    #self.P = self.defP + self.defP * (self.AUKF_Pmult_for_pert / (1 + self.learning_rate_P**(-np.sum(np.array(self.all_eps)[self.compensation_adaptive_frame_i_frm-accumulated_frames_threshold:]>sensory_error_threshold) ) ) - (self.AUKF_Pmult_for_pert/2) ) #self.AUKFmultFactor[2]
                    #self.R = self.defR + self.defR * (self.AUKF_Rmult_for_pert / (1 + self.learning_rate_R**(-np.sum(np.array(self.all_eps)[self.compensation_adaptive_frame_i_frm-accumulated_frames_threshold:]>sensory_error_threshold) ) ) - (self.AUKF_Rmult_for_pert/2) )
                    #self.R = self.defR + (1000) * (1.01) ** (-np.sum(np.array(self.all_eps)[self.compensation_adaptive_frame_i_frm-accumulated_frames_threshold:]>sensory_error_threshold) )  
                    #self.P = self.defP * ( (P_amp_test) / (1 + (1.13)**(-np.sum(np.array(self.all_eps)[self.compensation_adaptive_frame_i_frm-accumulated_frames_threshold:]>sensory_error_threshold) ) ) - (P_amp_test/2) ) #self.AUKFmultFactor[2]
                    ...
                    #pdb.set_trace()
                    #all_P_np = np.array(self.all_P)
                    #self.P = self.defP*np.var(all_P_np[-5:,:,:])
                    #self.Q = (self.defQ*0)

                    #self.R = self.defR + (1000)
                    #pdb.set_trace()
                    # self.R = self.defR * self.AUKF_Rmult_for_pert 
                    #pdb.set_trace()
...
def load_theta_and_x_training_samples():# Get list of all pickle files matching string
...
def get_closest_training_data_to_empirical(all_theta, all_x, empirical_obs, n=5):
    differences = abs(all_x - empirical_obs)
    differences_summed = torch.sum(differences, dim=1)
    sorted_tensor, indices = torch.sort(differences_summed)
    closest_thetas = all_theta[indices[0:n],:].T
    closest_xs = all_x[indices[0:n],:].T
    

    return closest_thetas, closest_xs
...
def main():
    ...
    all_theta, all_x = load_theta_and_x_training_samples()
    closest_theta_HC, closest_x_HC = get_closest_training_data_to_empirical(all_theta, all_x, healthy_mean_synth)
    closest_theta_CD, closest_x_CD = get_closest_training_data_to_empirical(all_theta, all_x, cereb_mean_synth)
    ...
    if True: #HC_or_CD == 'HC':
        ...
        if do_HC:
            # pdb.set_trace()
            posterior_modes_HC = closest_theta_HC[:,0]
            posterior_modes_HC[8] = 1000  # Rmult
            posterior_modes_HC[9] = 1e-15 # QMult
            posterior_modes_HC[10] = 3.5  # PMult

            posterior_modes_HC[11] = 1.01 # R lr 
            posterior_modes_HC[12] = 1.01 # Q lr
            posterior_modes_HC[13] = 1.13 # P lr

            posterior_modes_HC[14] = 100  # sensory error threshold

            ii = 0
            for string, value in zip(labels, posterior_modes_HC):
                print(f"{ii} {string:<20} | {value.item():<10.7f}")
                ii=ii++1
            # posterior_modes_HC = torch.tensor([0.005, 0.005, 0.01, 0.05, 0.0000001, 0.0000001, 155, 155, 1, 1, 1, 1.009, 1.009,1.009, 50]) 
            # pdb.set_trace()
            ...
            # Plot data
            # x_time = np.linspace(0,500, num=100)
            x_time = np.linspace(40, 140, num=100)
            ...
            plt.figure()
            plt.plot(x_time, mean_formants_HC, linestyle='--', color='blue', label='From manual test')
            # ci_HC = (1.96 * np.std(all_simulated_formant_HC,axis=0)/np.sqrt(all_simulated_formant_HC.shape[0]))[40:140]
            plt.fill_between(x_time, (mean_formants_HC-ci_HC), (mean_formants_HC+ci_HC), color='b', alpha=.1)
            plt.plot(x_time, healthy_mean_synth[40:140], linestyle='-', color='blue',label='HC Mean of observed trials')
            plt.fill_between(x_time, (healthy_mean_synth[40:140]-cereb_ci_ds), (healthy_mean_synth[40:140]+cereb_ci_ds), color='k', alpha=.1)
            plt.plot(x_time, closest_x_HC[40:140,0], 'red', label='Closest training')
            #     plt.axvline(x = 0, color = 'grey', ls=':')
            plt.legend()
            plt.xlabel('Time (ms)')
            plt.ylabel('Formant response (Hz)')
            ...
            plt.figure()
            x_time2 = np.linspace(0,140, num=140)
            if len(all_simulated_formant_HC.shape) > 1:
                all_frames_mean_formants_HC = np.mean(all_simulated_formant_HC, axis=0)
                all_frames_ci_HC = (1.96 * np.std(all_simulated_formant_HC,axis=0)/np.sqrt(all_simulated_formant_HC.shape[0]))
            if len(all_simulated_formant_HC.shape) == 1:
                all_frames_mean_formants_HC = all_simulated_formant_HC
                all_frames_ci_HC = 0
            plt.plot(x_time2, all_frames_mean_formants_HC, linestyle='--', color='blue' ,label='From closest training parameters')
            #plt.fill_between(x_time2, (all_frames_mean_formants_HC-all_frames_ci_HC), (all_frames_mean_formants_HC+all_frames_ci_HC), color='orange', alpha=.1)
            plt.plot(x_time2, healthy_mean_synth, linestyle='-', color='blue' ,label='CD Mean of observed trials')
            # plt.fill_between(x_time2, (healthy_mean_synth-all_frames_ci_HC), (cereb_mean_synth+all_frames_ci_HC), color='k', alpha=.1)
            plt.plot(x_time2, closest_x_HC[0:140,0], 'red', label='Closest training')
            plt.legend()
            plt.xlabel('Time (ms)')
            plt.ylabel('Formant response (Hz)')
            plt.show()
        ...
        if do_CD:
            posterior_modes_CD = closest_theta_CD[:,0]
            posterior_modes_CD[8] = 150  # Rmult
            posterior_modes_CD[9] = 1 # QMult
            posterior_modes_CD[10] = 8.15  # PMult

            posterior_modes_CD[11] = 1.01 # R lr 
            posterior_modes_CD[12] = 1.01 # Q lr
            posterior_modes_CD[13] = 1.06 # P lr

            posterior_modes_CD[14] = 100  # sensory error threshold

            ii = 0
            for string, value in zip(labels, posterior_modes_CD):
                print(f"{ii} {string:<20} | {value.item():<10.7f}")
                ii=ii++1
            ...
            # Plot data
            #x_time = np.linspace(0, 500, num=100)
            x_time = np.linspace(0, 140, num=100)
            ...
            plt.figure()
            plt.plot(x_time, mean_formants_CD, linestyle='--', color='orange' ,label='From manual test')
            ...
            plt.plot(x_time, closest_x_CD[40:140,0], 'red', label='Closest training')
            #     plt.plot(x_time, simulated_formant_manual_test[40:140], 'red',label='Manual test')
            #     plt.axvline(x = 0, color = 'grey', ls=':')
            plt.legend()
            ...
            # plt.figure()
            # all_frames_mean_formants_CD = all_simulated_formant_CD
            # all_frames_ci_CD = (1.96 * np.std(all_simulated_formant_CD,axis=0)/np.sqrt(all_simulated_formant_CD.shape[0]))
            # plt.plot(all_frames_mean_formants_CD, linestyle='--', color='orange' ,label='From closest training parameters')
            # plt.fill_between((all_frames_mean_formants_CD-all_frames_ci_CD), (all_t_mean_formants_CD+all_frames_ci_CD), color='orange', alpha=.1)
            # plt.plot(cereb_mean_synth, linestyle='-', color='orange' ,label='CD Mean of observed trials')
            # plt.fill_between((cereb_mean_synth-all_frames_ci_CD), (cereb_mean_synth+all_frames_ci_CD), color='k', alpha=.1)
            # plt.plot(closest_x_CD[0:140,0], 'red', label='Closest training')
            # plt.legend()
            # plt.xlabel('Time (ms)')
            # plt.ylabel('Formant response (Hz)')
            # plt.show()
