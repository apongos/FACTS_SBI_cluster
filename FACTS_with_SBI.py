...
def main(num_sim, num_workers):
    ...
    trial_cells_times = scipy.io.loadmat(sing_path+'/formant_pert_time_cleaned.mat')['time_matrix'].T
    trial_cells_mat = scipy.io.loadmat(sing_path+'/formant_pert_data_cleaned.mat')['cleaned_matrix'].T # 1797 x 194 == trials by time
    ...
    if load_instead == False:
        ...
        with open(sing_path+f'/ModelC_auditory_soma_noise_posterior_{num_sim}.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
            pickle.dump([parameter_posterior], f)
        
    else:
        file = open(sing_path+f'/ModelC_auditory_soma_noise_posterior_{num_sim}.pkl', 'rb')
